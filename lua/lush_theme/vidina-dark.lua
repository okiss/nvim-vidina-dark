...
local background = hsl(209, 90, 8)
local primary = hsl(10, 70, 70)
local complement = primary.rotate(180) -- TODO
local text = primary.mix(complement, 50)

local palette = {
  background = background,
  highlight = background.li(20).de(30),
  shadow = background.li(10),
  shadow_light = background.li(4),
  contrast_text = text.li(50),

  -- primary -> complement scale
  p3 = primary.mix(complement, 0),
  p2 = primary.mix(complement, 20),
  p1 = primary.mix(complement, 30),
  text = text,
  c1 = primary.mix(complement, 70),
  c2 = primary.mix(complement, 80),
  c3 = primary.mix(complement, 100),

  -- faded text
  f1 = text.mix(background.de(50), 25),
  f2 = text.mix(background.de(50), 50),
}
...
local theme = lush(function(injected_functions)
  ...
  return {
    ...
    CursorLine({ bg = palette.shadow_light }), -- Screen-line at the cursor, when 'cursorline' is set. Low-priority if foreground (ctermfg OR guifg) is not set.
    Directory({ fg = palette.p3 }), -- Directory names (and other special names in listings)
    DiffAdd({ bg = hsl(140, 40, 30) }), -- Diff mode: Added line |diff.txt|
    DiffChange({ bg = hsl(220, 40, 30) }), -- Diff mode: Changed line |diff.txt|
    DiffDelete({ bg = hsl(350, 40, 30) }), -- Diff mode: Deleted line |diff.txt|
    DiffText({ bg = hsl(220, 40, 50) }), -- Diff mode: Changed text within a changed line |diff.txt|
    ...
    IncSearch({ fg = palette.shadow, bg = hsl(30, 100, 50) }), -- 'incsearch' highlighting; also used for the text replaced with ":s///c"
    Substitute({ fg = palette.shadow, bg = hsl(120, 100, 50) }), -- |:substitute| replacement text highlighting
    LineNr({ fg = palette.f1, bg = palette.background }), -- Line number for ":number" and ":#" commands, and when 'number' or 'relativenumber' option is set.
    CursorLineNr({ fg = palette.text, bg = palette.background }), -- Like LineNr when 'cursorline' or 'relativenumber' is set for the cursor line.
    MatchParen({ bg = palette.background.li(50) }), -- Character under the cursor or just before it, if it is a paired bracket, and its match. |pi_paren.txt|
    ...
    NormalFloat({ fg = palette.text, bg = palette.shadow_light }), -- Normal text in floating windows.
    NormalNC({ Normal }), -- normal text in non-current windows
    Pmenu({ fg = palette.text, bg = palette.shadow }), -- Popup menu: Normal item.
    PmenuSel({ fg = palette.contrast_text, bg = palette.highlight, bold = true }), -- Popup menu: Selected item.
    ...
    DiagnosticVirtualTextError({ fg = DiagnosticError.fg, bg = hsl(0, 100, 5) }), -- Used for "Error" diagnostic virtual text.
    DiagnosticVirtualTextWarn({ fg = DiagnosticWarn.fg, bg = hsl(30, 100, 5) }), -- Used for "Warn" diagnostic virtual text.
    ...
    sym("@punctuation.bracket")({ fg = palette.contrast_text }), -- Delimiter
    -- sym"@constant"          { }, -- Constant
    sym("@constant.builtin")({ fg = palette.c2.hue(300), bg = hsl(300, 70, 8) }), -- Special
    -- sym"@constant.macro"    { }, -- Define
    -- sym"@define"            { }, -- Define
    -- sym"@macro"             { }, -- Macro
    sym("@string")({ fg = palette.c2, bg = hsl(190, 70, 8) }), -- String
    sym("@string.escape")({ fg = palette.p1 }), -- SpecialChar
    sym("@string.special")({ fg = palette.p1 }), -- SpecialChar
    -- sym"@character"         { }, -- Character
    -- sym"@character.special" { }, -- SpecialChar
    sym("@number")({ fg = palette.c2.hue(120), bg = hsl(120, 70, 8) }), -- Number
    sym("@boolean")({ fg = palette.c2.hue(300), bg = hsl(300, 70, 8) }), -- Boolean
    sym("@float")({ fg = palette.c2.hue(120), bg = hsl(120, 70, 8) }), -- Float
    sym("@function")({ fg = palette.p2, bold = true }), -- Function
    sym("@function.call")({ fg = Normal.fg, bold = false }), -- Function
    sym("@function.method.call")({ fg = Normal.fg, bold = false }), -- Function
    ...
    sym("@include")({ fg = palette.p3 }), -- Include
    -- sym"@preproc"           { }, -- PreProc
    -- sym"@debug"             { }, -- Debug
    sym("@tag")({ fg = palette.p2, bold = true }), -- Tag
    sym("@tag.attribute")({ fg = palette.c1 }), -- Tag
    ...
    --
    GitSignsAdd({ fg = hsl(140, 60, 40) }),
    GitSignsChange({ fg = hsl(220, 60, 50) }),
    GitSignsDelete({ fg = hsl(350, 60, 40) }),
    GitSignsCurrentLineBlame({ fg = palette.f2, italic = true, bold = false }),

    TelescopeNormal({ fg = Normal.fg, bg = hsl(210, 50, 14) }),
    TelescopeTitle({ fg = palette.f2, bg = hsl(210, 50, 14) }),
    TelescopeBorder({ fg = hsl(210, 50, 18), bg = hsl(210, 50, 14) }),
